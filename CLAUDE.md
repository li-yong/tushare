...
- **`us_weekly_run.sh`** — Sundays (`0 14 * * 0`). Slow-moving screens: `t_us_undervalue.py` (超跌优质, SP500∪NDX; primary) + `t_us_watchlist_suggest.py` (print-only — never `--apply` from cron) + `t_us_signal_attrib.py` (信号归因: forward outcomes for every episode in `result/us_signal_log/us_signal_ledger.csv`, which `t_us_tech_swing.py` appends to on live runs via `signal_ledger.py`). Logs to `us_weekly_run.log`.
