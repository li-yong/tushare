...
- **`us_daily_run.sh`** — weekdays (`0 14 * * 1-5`). Primary: `t_us_tech_swing.py` → Morning Report `result/us_tech_swing_<date>.txt`. Supplementary: three-layer chain `t_us_premium.py → t_us_delivery.py → t_us_resonance.py` (贵气×兑现×共振); `t_us_breakout_screen.py` (consolidation breakout); `t_us_steady_climb.py` (小步慢涨·跌一点快补回, NDX-100); `t_us_searchlight.py` (池外 贵气×兑现 侦察); `t_us_low_bounce.py --grok` (250日新低·首日反弹 候选发现层, 三道流动性门+市值门, 非验证信号源; --grok 对每个新命中跑一次 xAI 催化剂复核 ~$0.1, 只标注不门控, 前向样本积累在 `result/us_low_bounce/grok_catalyst.csv`); `t_us_earnings_react.py` (财报强反应·守住: 财报bar≤5交易日 + E/E+1收盘涨幅≥7% + 现价≥close(财报日-1), 参考止损=close(财报日-1); 价格漏斗先行故全池扫描不打爆财报日历API; 候选发现层, 可经 `huice --source earnings_react` 回放——过去公告日是事实无前视, caveat 是退市票缺日历的幸存者偏差); `t_us_key_kline.py --scan --universe all` (关键K线 择时扫描 → `us_key_kline_scan_<date>.txt`); `t_us_time_budget_annotate.py` (时间预算回填, registry #26: ledger episodes 按信号日体制预算 STRONG20/MIXED10/WEAK5 交易日期满后, 把 ⏳没动/✓已启动 判定追写进信号当日的 us_daily_report 末尾, 幂等, 跑在 daily-report 之后). Since 2026-07 the entry-candidate scans (key_kline / gap_scan / pullback_shock / tr_surge / trend_confirm / bottom_entry) run on `--universe all` = SP500 ∪ NDX ∪ Russell-2000 Health Care+Technology ∪ mid-cap $2B–$30B Health Care+Technology (~1290 names; R2000 slice = Vanguard VTWO holdings × Nasdaq screener sectors, midht slice = Nasdaq screener cap band — plugs the FROG-type gap between R2000's top and SP500's admission rules; both built in `t_us_undervalue.load_universe`); ranking/environment reads (market_leaders, breadth rs-ref, sector_rotation, network) stay on `both`. Last stage `t_us_futu_alert.py --commit` (Futu 到价提醒同步: 把当日报告四类关注价位——持仓有效止损/就绪入场setup失效线/共振≥3关键K线止损/启动火种live_stop——写成 Futu price reminder, 全 PRICE_DOWN·ONCE; 只管理 note 带 `[auto]` 标记的提醒, 全四源做一次 reconcile 补新/换价/离场清理, held 票用持仓止损排他不被候选形态收紧, 已失守/瞬触的跳过; 写实盘账户依赖 OpenD, 放在 daily-report 之后保证共振源同日, 非致命). Logs to `us_daily_run.log`.
- **`us_weekly_run.sh`** — Sundays (`0 14 * * 0`). Slow-moving screens: `t_us_undervalue.py` (超跌优质, SP500∪NDX; primary) + `t_us_watchlist_suggest.py` (print-only — never `--apply` from cron) + `t_us_news_top_detector.py` (新闻见顶稻草 + 启动火种: Grok top_scan × §2.1 exhaustion bar, 加 §6 镜像——SP500∪NDX 关键K线价格漏斗 → Grok launch_scan; ~$5/run, degrades to price-only without xAI; see `docs/news_driven_top_detection.md`) + `t_us_signal_attrib.py` (信号归因: forward outcomes for every episode in `result/us_signal_log/us_signal_ledger.csv`, which `t_us_tech_swing.py` and `t_us_news_top_detector.py` append to on live runs via `signal_ledger.py`; runs after news-top so fresh episodes are included) + `t_us_tide_gauge.py` (潮位仪·真水量: Fed H.4.1 净流动性 WALCL−TGA−RRP 免key官方zip + 全市场总市值 screener + ETF创赎流向前向积累; 报水量潮向与价格潮位的背离, 提示不门控, registry #22) + `t_us_wind_class.py` (阵风/季风分类: 相对线持续性×新鲜度五档 + Grok narrative_scan 叙事前向日志 ~$0.1, 无key降级价格端only, registry #23) + `t_us_swell.py` (无风有涌: 碾磨式跑赢价格漏斗 → Grok swell_scan 查无催化剂=SWELL标注 ~$0.8, 纯标注不门控, registry #24) + `us_weekly_report.py` (周报 merger, mirrors `us_daily_report.py`: merges every weekly sub-report into `result/weekly_report/us_weekly_report_<date>.md` — 水文环境潮/风、稻草/火种、慢筛候选共振计票、信号账本审计、周日复盘清单, 原文附录; runs last) + `t_us_futu_alert.py --commit` (Futu 到价提醒同步, 同 daily 末段; 周日跑主要把当周新鲜 news_top 火种 live_stop 及时写进提醒, 持仓/入场读周五最新 tech_swing). Logs to `us_weekly_run.log`.
...
- Run by hand: `ndx_predictor.py` (quarterly, 3/6/9/12, NDX rebalance); `t_us_key_kline.py` — `--ticker SYM` annotates one name (chart + 现状 timing/stop block + legend), or `--scan` sweeps a pre-filtered universe (`--universe sp500|ndx|both|r2000ht|all`, `--plot-top N`) for names at a fresh entry now, ranked. Not a free-market screener (方法论 §2.1: only quality pools have a real 主语).
- **`huice.py`** — 指示级回测 (run by hand): point-in-time replay of the scanners' directives (tech_swing / key_kline / gap_scan / undervalue / chanlun / tr_surge / earnings_react via `--source`), forward-simulated under the system's own exit discipline. `-asof <date> --ticker SYM` replays one day; `--start/--end [--universe sp500|ndx|both|r2000ht|all] [--jobs N]` sweeps a range (episodes deduped like the signal ledger). It reuses the live scanners' code through `tsw._ASOF` truncation — when adding a per-date memo, always re-anchor `_ASOF` on cache hits or later tickers leak the future. Findings: `docs/huice_backtest_findings.md` — headline: edge lives in the pool, not the patterns (all sources lose their α on SP500∪NDX); positive expectancy is regime-conditional (2022: all meanR negative, right tail gone, but the layered stops kept avg loss per episode under ~3.5%); MIXED-state entries are the one robust negative-EV pocket. Synthetic replay has no execution friction — the live ledger (`t_us_signal_attrib.py`) stays the source of truth. Live 规则的有效域与闭环状态登记在 `docs/rule_registry.md`(认识等级制: huice=假说, ledger=知识)— 任何复盘/回测/归因结论必须以「已改: 改了哪个门控/参数」或「明确不改+理由」收尾, 并更新该表。
...
- 市场三尺度认识框架见 `docs/tide_wave_wind.md`(潮=流动性水量/风=叙事能量/浪=价格形态;風顶潮=MIXED 负期望)— 设计或定位新指标时先问它测哪一层;姊妹心法 `docs/indicator_design_state_vs_debt.md`(GPS 不是讨债单)、`docs/trading_mindset_and_frequency.md`(反人性=快系统三内核、机会频率梯子 11天/年、仓位是输出)、`docs/panic_drawdown_empirics.md` §五-七(恐慌标尺三层阈值×新鲜度、加仓次序 指数>池>单票、恐慌镜像)、`docs/deep_drawdown_cohort_study.md`(深坑买入 cohort 期望为负)。
