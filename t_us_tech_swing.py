...
PANIC_THRESH = {            # 恐慌标尺·行为学阈值 = 历史 1%位 (1d/5d/21d 交易日收盘跌幅;
    'QQQ':  (-0.04, -0.08, -0.17),   # 指数档, ^IXIC 1971~ 校准
    'SOXX': (-0.06, -0.13, -0.24),   # 板块·半导体档, ^SOX 1994~ 校准
}                           # docs/panic_drawdown_empirics.md §六, registry #28。触发只说明
                            # "是恐慌", 可买性走晨报检查清单 — 提示不门控, 不改扫描行为。
PANIC_FRESH_D = 180         # 可买性一号检查: 距最近高点 ≤180 自然日 = 新鲜(急性)恐慌;
                            # 陈旧 = 出清瀑布段 (新鲜单日档 f252 中位 +33% vs 陈旧 -1%)。
...
def panic_ruler_check() -> list[dict]:
    """恐慌标尺: 对 PANIC_THRESH 里的每个 barometer 查 1d/5d/21d 收盘跌幅是否破
    行为学阈值, 返回触发列表 (空=无触发)。纯价格计算, --asof 下可重放 (数据层已按
    _ASOF 截断); "最近高点"取缓存窗口 (~3年) 内最高收盘 — 对新鲜度判定足够,
    比它更老的高点本来就该读陈旧。"""
    fired = []
    for ticker, thrs in PANIC_THRESH.items():
        df = _fetch_daily(ticker)
        if df.empty or len(df) < 22:
            continue
        c = df['close']
        fresh_d = (c.index[-1] - c.idxmax()).days
        for n, thr, label in zip((1, 5, 21), thrs, ('1d', '5d', '21d')):
            ret = float(c.iloc[-1] / c.iloc[-1 - n] - 1)
            if ret <= thr:
                fired.append({'ticker': ticker, 'window': label, 'ret': ret,
                              'thr': thr, 'fresh_d': fresh_d,
                              'fresh': fresh_d <= PANIC_FRESH_D,
...
def print_report(market_state, baro_info, results, pos_alerts, output_file=None,
    ...
    # ── 恐慌标尺 (§五加仓次序 × §六三层阈值; registry #27/#28, 提示不门控) ─────
    try:
        panic_fired = panic_ruler_check()
    except Exception as e:
        panic_fired = []
        logging.warning(f'panic ruler check failed: {e}')
    for f in panic_fired:
        tag = ('新鲜✓ 急性恐慌 → 适用加仓次序' if f['fresh']
               else '陈旧✗ 出清瀑布段 → 只用三档预案, 不按恐慌买')
        p(f"  🚨 恐慌标尺: {f['ticker']} {f['window']} {f['ret']:+.1%} ≤ {f['thr']:.0%}"
          f"  距高点 {f['fresh_d']} 自然日 ({tag}), 回撤 {f['dd']:+.1%}")
    if panic_fired:
        p('     □ 无差别性: 全市场同跌=潮(可按次序买) / 本板块独跌=风死(不买) — 对照等权大盘与池外')
        p('     □ 次序: 指数 > 池 > 单票 — 接个股刀多一个归零维度; 单票超跌残差=信息, 错杀须举证(grok)')
        p('     □ 预算: 新鲜恐慌 63d 再挖深中位 -5~-8% 仍在路上; 泡沫顶第一跌与牛市闪电同貌'
          '(2000-04 f252 -42%) → 只下等得回来的仓位')
